...
Mocks = require( "./AssertiveMock")
TickScheduler = require("./AssertiveTickScheduler").TickScheduler
mock = Mocks.mock
mockFunction = Mocks.mockFunction
EventEmitter = require("events").EventEmitter
th = require("./AssertiveSpecHelper")
t = th.t
...
expectError = (errorText, f) ->
  assert.throws f, Error, errorText

describe "Bacon._", ->
  _ = Bacon._
  describe "head", ->
    assert.equal _.head([5,2,9]), 5
    assert.equal _.head([]), undefined
    assert.equal _.head(5), undefined
  describe "always", -> assert.equal _.always(5)("francis"), 5
  describe "negate", ->
    assert.isFalse _.negate(_.always(true))("timanttikobra")
  describe "empty", ->
    assert.isTrue _.empty([])
    assert.isTrue _.empty("")
    assert.isFalse _.empty([1])
    assert.isFalse _.empty("1")
  describe "tail", ->
  ...
  describe "filter", ->
    assert.deepEqual _.filter(_.empty, ["","1",[],[2]]), ["",[]]
  describe "map", ->
    assert.deepEqual _.map(_.head, [[], [1], [2,2], [3,3,3] ]), [undefined, 1, 2, 3 ]
  describe "flatMap", ->
    assert.deepEqual _.flatMap(((x) -> [x, x]), [1,2,3]), [1,1,2,2,3,3]
  describe "each", ->
    it "provides key and value to iterator", ->
      expectKeyVals = (x, expectedKeys, expectedValues) ->
        ...
        assert.deepEqual [keys, values], [expectedKeys, expectedValues]
      expectKeyVals(
        {cat:"furry",bird:"feathery"}, ["cat","bird"], ["furry","feathery"]
      )
      expectKeyVals([1,2,3], ["0","1","2"], [1,2,3])
  describe "toArray", ->
    assert.deepEqual _.toArray(2), [2]
    it "ignores rest of arguments", ->
      assert.deepEqual _.toArray(1,1,2), [1]
    it "should, when given an array, return it back (not a copy)", ->
      arr = []
      assert.equal _.toArray(arr), arr
  describe "indexOf", ->
    assert.equal _.indexOf([1,2], 1), 0
    assert.equal _.indexOf([1,2], 2), 1
    assert.equal _.indexOf([1,2], 3), -1
  describe "contains", ->
    assert.isTrue _.contains("abc", "c")
    assert.isFalse _.contains("abc", "x")
    assert.isTrue _.contains([2,4,6], 4)
    assert.isFalse _.contains([2,4,6], 3)
  describe "id", ->
    obj = {}
    assert.equal _.id(obj), obj
  describe "last", ->
    assert.equal _.last([2,4]), 4
    assert.equal _.last("last"), "t"
  describe "all", ->
    assert.isFalse _.all([ [false,true], [true,true] ], _.head)
    assert.isTrue _.all([ [true,false], [true,true] ], _.head)
    it "should test truthiness if no function given", ->
      assert.isFalse _.all([true, false, true])
      assert.isTrue _.all([true, true, true])
      assert.isTrue _.all([1, true, 1])
  describe "any", ->
    assert.isTrue _.any([ [false,true], [true,true] ], _.head)
    assert.isFalse _.any([ [false,false], [false,true] ], _.head)
    it "should test truthiness if no function given", ->
      assert.isFalse _.any([false, false, false])
      assert.isTrue _.any([true, false, true])
  describe "without", ->
    assert.deepEqual _.without("apple", ["bacon","apple","apple","omelette"]), ["bacon","omelette"]
  describe "remove", ->
    assert.deepEqual _.remove("apple", ["bacon","apple","apple","omelette"]), ["apple"]
    assert.deepEqual _.remove("raisin", ["bacon","apple","apple","omelette"]), undefined
  describe "fold", ->
    assert.equal _.fold([1,2,3,4,5], 0, (s, n) -> s + n), 15
  describe "toString", ->
    it "for booleans", ->
      assert.equal _.toString(true), "true"
    it "for numbers", ->
      assert.equal _.toString(1), "1"
      assert.equal _.toString(1.1), "1.1"
    it "for undefined and null", ->
      assert.equal _.toString(undefined), "undefined"
      assert.equal _.toString(null), "undefined"
    it "for strings", ->
      assert.equal _.toString("lol"), "lol"
    it "for dates", ->
      assert.include _.toString(new Date((new Date(0)).getTimezoneOffset() * 60 * 1000)), "1970"
    it "for arrays", ->
      assert.equal _.toString([1,2,3]), "[1,2,3]"
    it "for objects", ->
      assert.equal _.toString({a: "b"}), "{a:b}"
      assert.equal _.toString({a: "b", c: "d"}), "{a:b,c:d}"
    it "for circular refs", ->
      obj = { name : "nasty" }
      obj.self = obj
      assert _.toString(obj).length < 100
    it "works even when enumerable properties throw errors on access", ->
      ...
      assert.equal _.toString(obj), "{name:madcow,prop:Error: an error}"
...
describe "Bacon.later", ->
  ...
  describe "supports sending an Error event as well", ->
    expectStreamEvents(
      ...
      [error()])
  it "toString", ->
    assert.equal Bacon.later(1, "wat").toString(), "Bacon.later(1,wat)"
  it "inspect", ->
    assert.equal Bacon.later(1, "wat").inspect(), "Bacon.later(1,wat)"
...
describe "Bacon.sequentially", ->
  ...
  describe "will stop properly even when exception thrown by subscriber", ->
    expectStreamEvents(
      ...
      ["lol", "wut"], unstable)
  it "toString", ->
    assert.equal Bacon.sequentially(1, [2]).toString(), "Bacon.sequentially(1,[2])"
...
describe "Bacon.repeatedly", ->
  describe "repeats given sequence forever", ->
    expectStreamEvents(
      ...
      [1,2,1,2,1])
  it "toString", ->
    assert.equal Bacon.repeatedly(1, [1]).toString(), "Bacon.repeatedly(1,[1])"
...
describe "Bacon.interval", ->
  describe "repeats single element indefinitely", ->
    expectStreamEvents(
      ...
      ["x", "x", "x"])
  it "toString", ->
    assert.equal Bacon.interval(1, 2).toString(), "Bacon.interval(1,2)"
...
describe "Bacon.fromPoll", ->
  describe "repeatedly polls given function for values", ->
    expectStreamEvents(
      ...
      ["lol", "lol"])
  it "toString", ->
    assert.equal Bacon.fromPoll(1, (->)).toString(), "Bacon.fromPoll(1,function)"
...
describe "Bacon.fromCallback", ->
  ...
  describe "supports object, methodName, partial application", ->
    expectStreamEvents(
      ...
      ["hello bob"])
  it "toString", ->
    assert.equal Bacon.fromCallback((->), "lol").toString(), "Bacon.fromCallback(function,lol)"
...
describe "Bacon.fromNodeCallback", ->
  ...
  describe "supports object, methodName, partial application", ->
    expectStreamEvents(
      ...
      ["hello bob"])
  it "toString", ->
    assert.equal Bacon.fromNodeCallback((->), "lol").toString(), "Bacon.fromNodeCallback(function,lol)"
...
describe "Bacon.fromEventTarget", ->
  ...
  it "should clean up event listeners from EventEmitter", ->
    ...
    emitter.emit "data", "x"
    assert.deepEqual emitter.listeners("data").length, 0
  ...
  it "should clean up event listeners from DOM object", ->
    ...
    dispose()
    assert.deepEqual emitter.listeners("click").length, 0
  ...
  it "toString", ->
    assert.equal Bacon.fromEventTarget({}, "click").toString(), "Bacon.fromEventTarget({},click)"
...
describe "Observable.log", ->
  ...
  it "logs event values as themselves (doesn't stringify)", ->
    value = {}
    assert.equal new Bacon.Next(value).log(), value
  it "logs Error, End events as strings", ->
    assert.equal new Bacon.Error("err").log(), "<error> err"
    assert.equal new Bacon.End().log(), "<end>"
  it "toString", ->
    assert.equal Bacon.never().log().toString(), "Bacon.never()"
...
describe "Observable.slidingWindow", ->
  ...
  it "toString", ->
    assert.equal Bacon.never().slidingWindow(2).toString(), "Bacon.never().slidingWindow(2,0)"
...
describe "EventStream.filter", ->
  ...
  describe "can filter by Property value", ->
    expectStreamEvents(
      ...
      [1,1,3,7])
  it "toString", ->
    assert.equal Bacon.never().filter(false).toString(), "Bacon.never().filter(function)"
...
describe "EventStream.map", ->
  ...
  it "toString", ->
    assert.equal Bacon.once(1).map(true).toString(), "Bacon.once(1).map(function)"
...
describe "EventStream.mapError", ->
  ...
  describe "also accepts a constant value", ->
    expectStreamEvents(
        ...
        [1, "ERR"])
  it "toString", ->
    assert.equal Bacon.never().mapError(true).toString(), "Bacon.never().mapError(function)"
...
describe "EventStream.doAction", ->
  ...
  describe "does not alter the stream", ->
    expectStreamEvents(
      ...
      [1, 2])
  it "toString", ->
    assert.equal Bacon.never().doAction((->)).toString(), "Bacon.never().doAction(function)"
...
describe "EventStream.mapEnd", ->
  ...
  describe "works with undefined value as well", ->
    expectStreamEvents(
      ...
      ["1", error(), undefined])
  it "toString", ->
    assert.equal Bacon.never().mapEnd(true).toString(), "Bacon.never().mapEnd(function)"
...
describe "EventStream.take", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      [1,2])
  it "toString", ->
    assert.equal Bacon.never().take(1).toString(), "Bacon.never().take(1)"
...
describe "EventStream.takeWhile", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      [1, 2])
  it "toString", ->
    assert.equal Bacon.never().takeWhile(true).toString(), "Bacon.never().takeWhile(function)"
...
describe "EventStream.skip", ->
  ...
  describe "works with synchronous source", ->
    ...
    [2, 3])
  it "toString", ->
    assert.equal Bacon.never().skip(1).toString(), "Bacon.never().skip(1)"
  ...
  describe "for synchronous sources", ->
    describe "skips filter predicate holds true", ->
      expectStreamEvents(
        ...
        [3, 2])
  it "toString", ->
    assert.equal Bacon.never().skipWhile(1).toString(), "Bacon.never().skipWhile(function)"
...
describe "EventStream.skipUntil", ->
  ...
  describe "works with self-derived starter with an evil twist", ->
    expectStreamEvents(
      ...
      [3])
  it "toString", ->
    assert.equal Bacon.never().skipUntil(Bacon.once(1)).toString(), "Bacon.never().skipUntil(Bacon.once(1))"
...
describe "EventStream.skipDuplicates", ->
  ...
  it "toString", ->
    assert.equal Bacon.never().skipDuplicates().toString(), "Bacon.never().skipDuplicates()"
...
describe "EventStream.flatMap", ->
  ...
  describe "works with a complex setup (fix #363)", ->
    it "case 1 (samplee has no subscribers)", ->
      ...
      assert.deepEqual result, {faq: "default value"}
  ...
  describe "Respects function construction rules", ->
    ...
    expectStreamEvents(
      ...
      ["sir", "francis"], unstable)
  it "toString", ->
    assert.equal Bacon.never().flatMap(->).toString(), "Bacon.never().flatMap(function)"
...
describe "EventStream.flatMapWithConcurrencyLimit", ->
  ...
  describe "Respects function construction rules", ->
    expectStreamEvents(
      ...
      ["sir francis"])
  it "toString", ->
    assert.equal Bacon.once(1).flatMapWithConcurrencyLimit(2, ->).toString(), "Bacon.once(1).flatMapWithConcurrencyLimit(2,function)"
...
describe "EventStream.flatMapConcat", ->
  ...
  describe "Respects function construction rules", ->
    expectStreamEvents(
      ...
      ["sir francis"])
  it "toString", ->
    assert.equal Bacon.once(1).flatMapConcat(->).toString(), "Bacon.once(1).flatMapConcat(function)"
...
describe "Property.flatMap", ->
  ...
  describe "works in a complex scenario #338", ->
    expectStreamEvents(
      ...
      ["ab", "Ab", "AB"], unstable)
  it "toString", ->
    assert.equal Bacon.constant(1).flatMap(->).toString(), "Bacon.constant(1).flatMap(function)"
...
describe "EventStream.flatMapLatest", ->
  ...
  describe "Accepts a field extractor string instead of function", ->
    ...
    expectStreamEvents(
      ...
      ["sir francis"])
  it "toString", ->
    assert.equal Bacon.never().flatMapLatest(->).toString(), "Bacon.never().flatMapLatest(function)"
...
describe "Property.flatMapLatest", ->
  ...
  describe "Accepts a constant EventStream/Property as an alternative to a function", ->
    expectStreamEvents(
      ...
      ["bacon"], unstable)
  it "toString", ->
    assert.equal Bacon.constant(1).flatMapLatest(->).toString(), "Bacon.constant(1).flatMapLatest(function)"
...
describe "EventStream.flatMapFirst", ->
  ...
  describe "Accepts a field extractor string instead of function", ->
    ...
    expectStreamEvents(
      ...
      ["sir francis"])
  it "toString", ->
    assert.equal Bacon.never().flatMapFirst(->).toString(), "Bacon.never().flatMapFirst(function)"
...
describe "EventStream.flatMapError", ->
  ...
  describe "has no effect on values", ->
    expectStreamEvents(
      ...
      [1, 2])
  it "toString", ->
    assert.equal Bacon.once(1).flatMapError(->).toString(), "Bacon.once(1).flatMapError(function)"
...
describe "EventStream.merge", ->
  ...
  describe "works with synchronous sources", ->
    expectStreamEvents(
      ...
      [1,2,3,4])
  it "toString", ->
    assert.equal Bacon.once(1).merge(Bacon.once(2)).toString(), "Bacon.once(1).merge(Bacon.once(2))"
...
describe "EventStream.delay", ->
  ...
  it "toString", ->
    assert.equal Bacon.never().delay(1).toString(), "Bacon.never().delay(1)"
...
describe "EventStream.debounce", ->
  ...
  describe "works in combination with scan", ->
    ...
    expectPropertyEvents(
      ...
      [0, 1, 3, 6]
      {extraCheck: -> it "calls function once per value", -> assert.equal count, 3}
    )
  it "toString", ->
    assert.equal Bacon.never().debounce(1).toString(), "Bacon.never().debounce(1)"
...
describe "EventStream.debounceImmediate(delay)", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      [1])
  it "toString", ->
    assert.equal Bacon.never().debounceImmediate(1).toString(), "Bacon.never().debounceImmediate(1)"
...
describe "EventStream.throttle(delay)", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      [3])
  it "toString", ->
    assert.equal Bacon.never().throttle(1).toString(), "Bacon.never().throttle(1)"
...
describe "EventStream.bufferingThrottle(minimumInterval)", ->
  describe "limits throughput but includes all events", ->
    th.expectStreamTimings(
      ...
      [[1,1], [4,2], [7,3]])
  it "toString", ->
    assert.equal Bacon.once(1).bufferingThrottle(2).toString(), "Bacon.once(1).bufferingThrottle(2)"
...
describe "EventStream.holdWhen", ->
  ...
  it "toString", ->
    assert.equal Bacon.once(1).holdWhen(Bacon.constant(true)).toString(), "Bacon.once(1).holdWhen(Bacon.constant(true))"
...
describe "EventStream.bufferWithTime", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      [[1,2,3]])
  it "toString", ->
    assert.equal Bacon.never().bufferWithTime(1).toString(), "Bacon.never().bufferWithTime(1)"
...
describe "EventStream.bufferWithCount", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      [[1, 2], [3, 4], [5]])
  it "toString", ->
    assert.equal Bacon.never().bufferWithCount(1).toString(), "Bacon.never().bufferWithCount(1)"
...
describe "EventStream.bufferWithTimeOrCount", ->
  ...
  describe "flushes on timeout", ->
    expectStreamEvents(
      ...
      [error(), [1, 2, 3, 4], [5, 6, 7]])
  it "toString", ->
    assert.equal Bacon.never().bufferWithTimeOrCount(1, 2).toString(), "Bacon.never().bufferWithTimeOrCount(1,2)"
...
describe "EventStream.takeUntil", ->
  ...
  describe "unsubscribes its source as soon as possible", ->
     expectStreamEvents(
       ->
        ...
        Bacon.later(20)
        .onUnsub(->
          assert.equal sc.now(), startTick + 1)
  ...
  ###
  it "toString", ->
    assert.equal Bacon.later(1, "a").takeUntil(Bacon.later(2, "b")).toString(), "Bacon.later(1,a).takeUntil(Bacon.later(2,b))"
...
describe "EventStream.awaiting(other)", ->
  ...
  describe "supports simultaneouts events", ->
    ...
    expectPropertyEvents(
      ...
      [false])
  it "toString", ->
    assert.equal Bacon.never().awaiting(Bacon.once(1)).toString(), "Bacon.never().awaiting(Bacon.once(1))"
...
describe "EventStream.endOnError", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      [1, 2, error()])
  it "toString", ->
    assert.equal Bacon.never().endOnError().toString(), "Bacon.never().endOnError()"
...
describe "Bacon.constant", ->
  ...
  it "provides same value to all listeners, when mapped (bug fix)", ->
    ...
    f.verify("lol")
  it "toString", ->
    assert.equal Bacon.constant(1).toString(), "Bacon.constant(1)"
...
describe "Bacon.fromArray", ->
  ...
  describe "Allows wrapped events, for instance, Bacon.Error", ->
    expectStreamEvents(
      ...
      [error(), 1])
  it "toString", ->
    assert.equal Bacon.fromArray([1,2]).toString(), "Bacon.fromArray([1,2])"
  it "doesn't mutate the given array, toString works after subscribe (bug fix)", ->
    array = [1,2]
    s = Bacon.fromArray(array)
    s.onValue(->)
    assert.equal s.toString(), "Bacon.fromArray([1,2])"
...
describe "EventStream.concat", ->
  ...
  describe "Works with synchronized left stream and doAction", ->
    expectStreamEvents(
      ...
      [2])
  it "toString", ->
    assert.equal Bacon.once(1).concat(Bacon.once(2)).toString(), "Bacon.once(1).concat(Bacon.once(2))"
...
describe "EventStream.startWith", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      ['pow', 1, 2, 3], unstable)
  it "toString", ->
    assert.equal Bacon.never().startWith(0).toString(), "Bacon.never().startWith(0)"
...
describe "Property.startWith", ->
  ...
  it "toString", ->
    assert.equal Bacon.constant(2).startWith(1).toString(), "Bacon.constant(2).startWith(1)"
...
describe "EventStream.toProperty", ->
  ...
  it "preserves laziness", ->
    ...
    assert.equal calls, 1
  it "toString", ->
    assert.equal Bacon.once(1).toProperty(0).toString(), "Bacon.once(1).toProperty(0)"
...
describe "Property.takeUntil", ->
  ...
  it "toString", ->
    assert.equal Bacon.constant(1).takeUntil(Bacon.never()).toString(), "Bacon.constant(1).takeUntil(Bacon.never())"
...
describe "Property.delay", ->
  ...
  describe "does not delay initial value", ->
    expectPropertyEvents(
      ...
      [0])
  it "toString", ->
    assert.equal Bacon.constant(0).delay(1).toString(), "Bacon.constant(0).delay(1)"
...
describe "Property.debounce", ->
  ...
  it "toString", ->
    assert.equal Bacon.constant(0).debounce(1).toString(), "Bacon.constant(0).debounce(1)"
describe "Property.throttle", ->
  ...
  describe "works with Bacon.once (bug fix)", ->
    expectPropertyEvents(
      ...
      [1])
  it "toString", ->
    assert.equal Bacon.constant(0).throttle(1).toString(), "Bacon.constant(0).throttle(1)"
...
describe "Property.combine", ->
  ...
  describe "does not duplicate same error from two streams", ->
    expectPropertyEvents(
      ...
      [["same", "same"], error()])
  it "toString", ->
    assert.equal Bacon.constant(1).combine(Bacon.constant(2), (->)).toString(), "Bacon.constant(1).combine(Bacon.constant(2),function)"
...
describe "Bacon.groupSimultaneous", ->
  ...
  describe "works with synchronous sources", ->
      ...
      expectStreamEvents(
        ...
        [[[1]], [[2]], [[3]]])
  it "toString", ->
    assert.equal Bacon.groupSimultaneous(Bacon.never()).toString(), "Bacon.groupSimultaneous(Bacon.never())"
...
describe "When an Event triggers another one in the same stream, while dispatching", ->
  it "Delivers triggered events correctly", ->
    ...
    bus.push "a"
    bus.push "b"
    assert.deepEqual values, ["a", "A", "A", "B", "B", "b"]
  ...
  it "complex scenario (fix #470)", ->
    ...
    bus2.push false
    bus2.push true
    assert.deepEqual values, [["p1", true],["p1", false],["p1", true],["empty", true]]
...
describe "Bacon.combineAsArray", ->
  describe "initial value", ->
    ...
    it "is output as Initial event", ->
      assert.equal event.isInitial(), true
  ...
  it "preserves laziness", ->
    ...
    assert.equal calls, 1
  it "toString", ->
    assert.equal Bacon.combineAsArray(Bacon.never()).toString(), "Bacon.combineAsArray(Bacon.never())"
...
describe "Bacon.combineWith", ->
  ...
  describe "works with 0 inputs (results to a constant)", ->
    expectPropertyEvents(
      ...
      [1])
  it "toString", ->
    assert.equal Bacon.combineWith((->), Bacon.never()).toString(), "Bacon.combineWith(function,Bacon.never())"
  ...
  describe "accepts constants instead of properties", ->
    ...
    describe "true or false", ->
      expectPropertyEvents(
        ...
        [true])
  it "toString", ->
    assert.equal Bacon.constant(1).and(Bacon.constant(2).not()).or(Bacon.constant(3)).toString(), "Bacon.constant(1).and(Bacon.constant(2).not()).or(Bacon.constant(3))"
...
describe "Bacon.mergeAll", ->
  ...
  describe "returns empty stream for zero input", ->
    ...
    expectStreamEvents(
      ...
      [])
  it "toString", ->
    assert.equal Bacon.mergeAll(Bacon.never()).toString(), "Bacon.mergeAll(Bacon.never())"
...
describe "Property.sampledBy(stream)", ->
  ...
  describe "laziness", ->
    ...
    it "preserves laziness", ->
      assert.equal calls, 1
  it "toString", ->
    assert.equal Bacon.constant(0).sampledBy(Bacon.never()).toString(), "Bacon.constant(0).sampledBy(Bacon.never(),function)"
...
describe "Property.sample", ->
  ...
  describe "works with synchronous source", ->
    expectStreamEvents(
      ...
      [1, 1, 1, 1])
  it "toString", ->
    assert.equal Bacon.constant(0).sample(1).toString(), "Bacon.constant(0).sample(1)"
...
describe "EventStream.errors", ->
  describe "Includes errors only", ->
    expectStreamEvents(
      ...
      [error()])
  it "toString", ->
    assert.equal Bacon.never().errors().toString(), "Bacon.never().errors()"
  ...
  describe "calls accumulator function once per value", ->
    describe "(simple case)", ->
      ...
      expectPropertyEvents(
        ...
        [0, 1, 3, 6]
        { extraCheck: -> it "calls accumulator once per value", -> assert.equal count, 3}
      )
...
describe "EventStream.diff", ->
  ...
  it "toString", ->
    assert.equal Bacon.once(1).diff(0, (->)).toString(), "Bacon.once(1).diff(0,function)"
...
describe "EventStream.zip", ->
  ...
  describe "can zip an observable with itself", ->
    expectStreamEvents(
      ...
      [['a', 'b'], ['b', 'c']])
  it "toString", ->
    assert.equal Bacon.never().zip(Bacon.once(1)).toString(), "Bacon.never().zip(Bacon.once(1))"
...
describe "Bacon.zipAsArray", ->
  ...
  describe "works with 0 streams (=Bacon.never())", ->
    ...
    expectStreamEvents(
      ...
      [])
  it "toString", ->
    assert.equal Bacon.zipAsArray(Bacon.never(), Bacon.never()).toString(), "Bacon.zipAsArray(Bacon.never(),Bacon.never())"
...
describe "Bacon.zipWith", ->
  ...
  describe "works with 0 streams (=Bacon.never())", ->
    ...
    expectStreamEvents(
      ...
      [])
  it "toString", ->
    assert.equal Bacon.zipWith((->), Bacon.never()).toString(), "Bacon.zipWith(function,Bacon.never())"
...
describe "Bacon.when", ->
  ...
  describe "works with synchronous sources", ->
    expectStreamEvents(
      ...
      ["xy"])
  it "toString", ->
    assert.equal Bacon.when([Bacon.never()], (->)).toString(), "Bacon.when([Bacon.never()],function)"
...
describe "Bacon.update", ->
  ...
  it "toString", ->
    assert.equal Bacon.update(0, [Bacon.never()], (->)).toString(), "Bacon.update(0,[Bacon.never()],function)"
...
describe "combineTemplate", ->
  ...
  it "supports arrays", ->
    ...
    Bacon.combineTemplate(value).onValue (x) ->
      ...
      assert.deepEqual x.key instanceof Array, true # seems that the former passes even if x is not an array
    value = [{ x: 1 }, { x: 2 }]
    Bacon.combineTemplate(value).onValue (x) ->
      assert.deepEqual x, value
      assert.deepEqual x instanceof Array, true
    value = {key: [{ x: 1 }, { x: 2 }], key2: {}}
    Bacon.combineTemplate(value).onValue (x) ->
      assert.deepEqual x, value
      assert.deepEqual x.key instanceof Array, true
    value = {key: [{ x: 1 }, { x: Bacon.constant(2) }]}
    Bacon.combineTemplate(value).onValue (x) ->
      assert.deepEqual x, {key: [{ x: 1 }, { x: 2 }]}
      assert.deepEqual x.key instanceof Array, true # seems that the former passes even if x is not an array
  it "supports nulls", ->
    value = {key: null}
    Bacon.combineTemplate(value).onValue (x) ->
  ...
  it "supports NaNs", ->
    value = {key: NaN}
    Bacon.combineTemplate(value).onValue (x) ->
      assert.deepEqual isNaN(x.key), true
  it "supports dates", ->
    value = {key: new Date()}
    Bacon.combineTemplate(value).onValue (x) ->
      assert.deepEqual x, value
  ...
  it "supports functions", ->
    ...
    Bacon.combineTemplate(value).onValue (x) ->
      assert.deepEqual x, value
  it "toString", ->
    assert.equal Bacon.combineTemplate({ thing: Bacon.never(), const: "a" }).toString(), "Bacon.combineTemplate({thing:Bacon.never(),const:a})"
...
describe "Property.decode", ->
  describe "switches between source Properties based on property value", ->
    expectPropertyEvents(
      ...
      ["a", "b", "c"])
  it "toString", ->
    assert.equal Bacon.constant(1).decode({1: "lol"}).toString(), "Bacon.constant(1).decode({1:lol})"
...
describe "Bacon.Bus", ->
  ...
  it "unsubscribes inputs on end() call", ->
    ...
    bus.end()
    input.push("b")
    assert.deepEqual toValues(events), ["a", "<end>"]
  ...
  it "does not plug after end() call", ->
    ...
    bus.onValue(->)
    assert.deepEqual plugged, false
  ...
  it "toString", ->
    assert.equal new Bacon.Bus().toString(), "Bacon.Bus()"
...
describe "EventStream", ->
  ...
  it "handles one subscriber added twice just like two separate subscribers (case Bacon.noMore)", ->
    ...
    bus.subscribe(f)
    bus.push("bacon")
    assert.deepEqual values, ["bacon", "bacon"]
...
describe "Bacon.fromBinder", ->
  ...
  it "toString", ->
    assert.equal Bacon.fromBinder(->).toString(), "Bacon.fromBinder(function,function)"

describe "String presentations", ->
  describe "Initial(1).toString", ->
    it "is 1", ->
      assert.equal new Bacon.Initial(1).toString(), "1"
  describe "Next({a:1i}).toString", ->
    it "is {a:1}", ->
      assert.equal new Bacon.Next({a:1}).toString(), "{a:1}"
  describe "Error({a:1}).toString", ->
    it "is <error> {a:1}", ->
      assert.equal new Bacon.Error({a:1}).toString(), "<error> {a:1}"
  describe "End.toString", ->
    it "is <end>", ->
      assert.equal new Bacon.End().toString(), "<end>"
  describe "inspect", ->
    it "is the same as toString", ->
      assert.equal new Bacon.Initial(1).inspect(), "1"

describe "Observable.name", ->
  it "sets return value of toString and inspect", ->
    assert.equal Bacon.once(1).name("one").toString(), "one"
    assert.equal Bacon.once(1).name("one").inspect(), "one"
  it "modifies the stream in place", ->
    obs = Bacon.once(1)
    obs.name("one")
    assert.equal obs.toString(), "one"
  it "supports composition", ->
    assert.equal Bacon.once("raimohanska").name("raimo").take(1).inspect(), "raimo.take(1)"

describe "Observable.withDescription", ->
  it "affects toString and inspect", ->
    assert.equal Bacon.once(1).withDescription(Bacon, "una", "mas").inspect(), "Bacon.una(mas)"
  it "affects desc", ->
    description = Bacon.once(1).withDescription(Bacon, "una", "mas").desc
    assert.equal description.context, Bacon
    assert.equal description.method, "una"
    assert.deepEqual description.args, ["mas"]
...
describe "Bacon.spy", ->
  testSpy = (expectedCount, f) ->
    ...
    Bacon.spy spy
    f()
    assert.equal calls, expectedCount
