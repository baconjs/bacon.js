...
<a name="bacon-sampledby"></a>
[`Bacon.sampledBy(props, samplers, f)`](#bacon-sampledby "Bacon.sampledBy(props, samplers, f)") combines the latest values of given n `props` using the given n-ary function
...
You have probably noticed that similar EventStream can be created using [`Bacon.when`](#bacon-when):

```js
Bacon.when([ a.toEventStream(), b.toEventStream(), mult.toProperty() ], f)
```

But here is an important difference: [`Bacon.when`](#bacon-when) synchronizes all streams,
...
the next value (just like in [`Bacon.zipWith`](#bacon-zipwith)):
...
<a name="bacon-samplebyasarray"></a>
[`Bacon.sampleByAsArray(props, samplers)`](#bacon-samplebyasarray "Bacon.sampleByAsArray(props, samplers)") is just like [`Bacon.sampledBy`](#bacon-sampledby), but combines the latest values of all `props` into an array.
