...
describe "Bacon.fromEvent", ->
  ...
  describe "eventSource is a string (the name of the event type to bind)", ->
  ...
  describe "eventSource is a function (a custom bind/unbind handler)", ->
    describe "should create EventStream from DOM object", ->
      expectStreamEvents(
        ->
          emitter = new EventEmitter()
          emitter.on "newListener", ->
            soon -> emitter.emit "click", "x"
          element = toEventTarget emitter
          take 1, Bacon.fromEvent(element, (binder, listener) => binder("click", listener))
        ["x"]
      )
    ...
    describe "should create EventStream from EventEmitter", ->
      expectStreamEvents(
        ->
          ...
          emitter.on "newListener", ->
            soon -> emitter.emit "data", "x"
          take 1, Bacon.fromEvent(emitter, (binder, listener) => binder("data", listener))
        ["x"]
      )
    ...
    it "should clean up event listeners from EventEmitter", ->
      emitter = new EventEmitter()
      take(1, Bacon.fromEvent(emitter, (binder, listener) => binder("data", listener))).subscribe ->
      emitter.emit "data", "x"
      expect(emitter.listeners("data").length).to.deep.equal(0)
    ...
    it "should clean up event listeners from DOM object", ->
      emitter = new EventEmitter()
      element = toEventTarget emitter
      dispose = Bacon.fromEvent(element, (binder, listener) => binder("click", listener)).subscribe ->
      dispose()
      expect(emitter.listeners("click").length).to.deep.equal(0)
    ...
    it "should create EventStream from on/off event", ->
      values = []
      src = onOffSource()
      take(1, Bacon.fromEvent(src, (binder, listener) => binder("test", listener))).onValue (value) ->
        values.push(value)
      deferred ->
        expect(values).to.deep.equal ["test"]
        expect(src.cleaned).to.equal(true)
    ...
    it "toString", ->
      expect(Bacon.fromEvent(onOffSource(), (binder, listener) => binder("click", listener)).toString())
        .to.equal("Bacon.fromEvent({on:function,off:function},function)")
