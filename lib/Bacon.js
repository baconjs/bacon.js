(function() {
  var Bacon, Bus, Dispatcher, End, Error, Event, EventStream, Initial, Next, Observable, Property, PropertyDispatcher, always, assert, assertArray, assertEvent, assertFunction, cloneArray, cloneObject, contains, empty, end, filter, former, head, initial, isEvent, isFunction, latter, map, next, nop, remove, tail, toEvent, toExtractor, _ref,
    _this = this,
  ...
  Observable = (function() {
    ...
    Observable.prototype.map = function(f) {
      ...
      return this.withHandler(function(event) {
        return this.push(event.fmap(extractor));
      });
    };
  ...
  toExtractor = function(f) {
    ...
    if (isFunction(f)) {
      return f;
    } else if ((typeof f === "string") && f.length > 1 && f[0] === ".") {
      key = f.slice(1);
